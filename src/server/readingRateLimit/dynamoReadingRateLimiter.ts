...
export class DynamoReadingRateLimiter {
  ...
  private async read(ref: string): Promise<Item | undefined> {
    try {
      return (await this.sender.send(new GetItemCommand({ TableName: this.config.tableName, Key: { rate_limit_ref: S(ref) }, ConsistentRead: true }))).Item;
    } catch (error) {
  ...
  async prepareAcquire(params: { userId: string; tier: MembershipTier; mode: RateLimitReadingMode; requestRef: string; ownerToken: string; now: Date }) {
    ...
    const rateRef = createRateWindowRef({ userId: params.userId, tier: params.tier, mode: params.mode, windowStart, secret: this.config.hashSecret });
    const previous = await this.read(rateRef);
    let used = 0; let version = 0; let createdAt = params.now.toISOString();
    if (previous) {
      if (text(previous, "schema_version") !== "shirone-reading-rate-window-v1" || text(previous, "scope") !== scope ||
          ...
          !Number.isSafeInteger(number(previous, "version")) || number(previous, "version") < 1) throw new ServerFoundationError("READING_RATE_LIMIT_INCONSISTENT");
      used = number(previous, "used"); version = number(previous, "version"); createdAt = text(previous, "created_at");
    }
    if (used >= policy.max) throw new ServerFoundationError("READING_RATE_LIMIT_REACHED", { retryAfter: Math.max(1, windowEnd - nowEpoch) });
    const rateAction: TransactWriteItem = { Put: {
      TableName: this.config.tableName,
      Item: { rate_limit_ref: S(rateRef), schema_version: S("shirone-reading-rate-window-v1"), scope: S(scope), window_start_epoch: N(windowStart), window_seconds: N(policy.windowSeconds), limit: N(policy.max), used: N(used + 1), version: N(version + 1), created_at: S(createdAt), updated_at: S(params.now.toISOString()), expires_at: N(windowEnd + policy.windowSeconds) },
    ...
    const reservation: RateControlReservation = { rateRef, rateWindowEnd: windowEnd };
    const actions = [rateAction];
    if (params.mode === "light" || params.mode === "deep") {
      if (!params.ownerToken || params.ownerToken.length > 128 || /[\u0000-\u001f\u007f]/u.test(params.ownerToken)) {
      ...
      let currentVersion = 0; let concurrencyCreatedAt = params.now.toISOString(); let active: ConcurrencyReservation[] = [];
      let expiredCount = 0;
      if (current) {
        if (text(current, "schema_version") !== "shirone-reading-concurrency-v1" || text(current, "scope") !== params.mode || number(current, "limit") !== 1 || !Number.isSafeInteger(number(current, "version")) || number(current, "version") < 1) throw new ServerFoundationError("READING_RATE_LIMIT_INCONSISTENT");
        ...
        const parsed = list.L.map((entry) => {
          ...
          return { requestRef: text(entry.M, "request_ref"), reservationId: text(entry.M, "reservation_id"), reservedAt: text(entry.M, "reserved_at"), expiresAt: number(entry.M, "expires_at") };
        });
        if (parsed.some((entry) => !/^[0-9a-f]{64}$/u.test(entry.requestRef) || !entry.reservationId || !entry.reservedAt || !Number.isSafeInteger(entry.expiresAt))) throw new ServerFoundationError("READING_RATE_LIMIT_INCONSISTENT");
        active = parsed.filter((entry) => entry.expiresAt > nowEpoch);
        expiredCount = parsed.length - active.length;
        currentVersion = number(current, "version"); concurrencyCreatedAt = text(current, "created_at");
      }
      ...
      const next = reusable ?? { requestRef: params.requestRef, reservationId: params.ownerToken, reservedAt: params.now.toISOString(), expiresAt: nowEpoch + this.config.concurrency.leaseSeconds };
      actions.push({ Put: {
        TableName: this.config.tableName,
        Item: { rate_limit_ref: S(concurrencyRef), schema_version: S("shirone-reading-concurrency-v1"), scope: S(params.mode), limit: N(1), reservations: { L: [{ M: { request_ref: S(next.requestRef), reservation_id: S(next.reservationId), reserved_at: S(next.reservedAt), expires_at: N(next.expiresAt) } }] }, version: N(currentVersion + 1), created_at: S(concurrencyCreatedAt), updated_at: S(params.now.toISOString()), expires_at: N(next.expiresAt + this.config.concurrency.leaseSeconds) },
      ...
      reservation.concurrencyRef = concurrencyRef; reservation.concurrencyReservationId = next.reservationId; reservation.concurrencyExpiresAt = next.expiresAt;
      if (!reusable && expiredCount > 0) reservation.concurrencyExpiredReclaimed = true;
    }
    return { reservation, actions, retryAfter: Math.max(1, windowEnd - nowEpoch) };
  }

  async prepareRelease(reservation: RateControlReservation, requestRef: string, now: Date): Promise<TransactWriteItem | undefined> {
