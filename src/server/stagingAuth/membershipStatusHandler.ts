...
export const STAGING_MEMBERSHIP_ROUTE_KEY = "GET /membership/status";
export const STAGING_MEMBERSHIP_OPTIONS_ROUTE_KEY = "OPTIONS /membership/status";
...
type Config = { enabled: boolean; allowedOrigins: ReadonlySet<string> };
...
export function createStagingMembershipStatusHandler(config: Config, dependencies: Dependencies) {
  return async (event: ApiGatewayV2Event): Promise<LambdaHttpResponse> => {
    ...
    try {
      ...
      const expectedRoute = request.method === "GET" ? STAGING_MEMBERSHIP_ROUTE_KEY : STAGING_MEMBERSHIP_OPTIONS_ROUTE_KEY;
      ...
      if (!config.enabled) throw new ServerFoundationError("STAGING_MEMBERSHIP_STATUS_DISABLED");
      ...
      writeSafeAuditLog({
        event: {
          requestId,
          event: "staging_membership_status_rejected",
