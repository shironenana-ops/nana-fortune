...
鑑定準備層では `READING_REQUEST_INVALID`、`READING_INPUT_TOO_LONG`、`READING_BIRTH_DATE_INVALID`、`READING_MODE_INVALID`、`READING_MODE_NOT_AVAILABLE`、`IDEMPOTENCY_KEY_REQUIRED`、`IDEMPOTENCY_KEY_INVALID` を共通固定エラーへ追加しました。入力形式は400、正しいmode名だが権利がない場合は403です。
