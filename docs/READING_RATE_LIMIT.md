...
具体的な回数と時間窓は商品・コスト方針として未確定です。コードにproduction defaultはありません。APIを有効化する場合、次の値をすべて明示し、不足・空・小数・負数・0・範囲外・unsafe integerは`READING_RATE_LIMIT_NOT_CONFIGURED`でfail closedします。
...
light/deep concurrencyは1以外を拒否します。テスト値をproduction値へ転用してはいけません。
...
RATE_LIMIT_POLICY_VALUES: BLOCKED_BY_RATE_LIMIT_POLICY_VALUES
LIMITED_PAID_BETA_GATE: BLOCKED
```

production値の正式決定、DynamoDB/IAM/Lambda設定、staging検証、限定公開承認が完了するまで一般開放しません。`READING_GENERATE_API_ENABLED`と`READING_DEEP_GENERATE_API_ENABLED`は、この実装作業では設定しません。
