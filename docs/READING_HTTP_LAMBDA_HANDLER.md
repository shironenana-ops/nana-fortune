...
12. 自前エンジンを1回実行
13. light／deepだけ既存Bedrock rendererを試行
14. 公開DTOをallow-listで新規構築
15. PIIを含まない監査ログとCORS／request ID付き応答を返す
...
- Idempotency-Keyは形式検証のみ。永続record、重複応答、処理状態は未実装
- history保存なし
- completed履歴と権利確定の原子性なし
