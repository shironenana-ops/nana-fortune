...
## Phase B反映（2026-07-23）

Phase Aの調査・原価試算・単一model設定という履歴は上記に保持する。Phase Bでは、承認された限定β候補をproductionへ有効化せず、次だけをローカル実装へ反映した。

- lightとdeepのmodel ID／aliasを別々の環境変数へ分離
- canonical `mode`だけを信頼してlight/deep modelを選択
- 旧`BEDROCK_MODEL_ID`／`BEDROCK_MODEL_ALIAS`をfallbackとして不使用
- free／未知modeがrendererへ到達した場合は送信前にfail closed
- Rate Limit 6 scopeとconcurrency 1/1を承認済みartifactへ固定
- runtimeは承認artifactを自動読込せず、production defaultを持たない

staging候補はlightが`jp.anthropic.claude-haiku-4-5-20251001-v1:0`、deepが`jp.anthropic.claude-sonnet-4-5-20250929-v1:0`、source regionが`ap-northeast-1`、processing scopeが`JAPAN`である。Global inference profileは使用しない。これらは非秘密の設定候補であり、AWS環境やproductionには未適用である。
...
次のgateは、staging用infra/IAM/環境変数の人間承認、mode別smoke、token・latency・fallback・429・concurrency leaseの実測、予算監視、kill switch確認である。本変更では`READING_GENERATE_API_ENABLED`、`READING_DEEP_GENERATE_API_ENABLED`、`READING_BEDROCK_ENABLED`を設定しない。
