...
`Idempotency-Key` は小文字canonical表現のUUID v4を1操作につき1つ必須とします。前後空白、大文字、複数値、カンマ結合値は拒否します。生keyは保存せず、利用者単位のHMAC request referenceとして予約します。同一key・同一入力のCOMPLETED再送は保存済み履歴を返し、異なる入力は409、有効lease中は409です。
...
HTTP層の後段へ渡す内部コマンドは、検証済み入力、token由来userId、解決済みmode、サーバー日付だけで新規構築します。raw body、会員item、メール、token、gender、履歴属性を含めません。現在の同期serviceは予約勝者だけ自前エンジンを1回実行し、公開DTOを構築した後、historyとidempotencyをtransactionで確定します。

## Success response

現在実装されている同期handlerは、成功時に200と`X-Request-Id`を返し、bodyにも同じ`request_id`を含めます。内部engine objectをspreadせず、次のallow-listだけを構築します。light／deepの限定β向け推奨契約は非同期202であり、[READING_ASYNC_API_CONTRACT_PROPOSAL.md](./READING_ASYNC_API_CONTRACT_PROPOSAL.md)に未実装案として分離しています。
