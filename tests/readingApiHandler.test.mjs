...
function setup({ enabled = true, membership = {}, repositoryError, beginError, rendererMode = "rendered", persistenceKind = "acquired", deepEnabled = true, deepReservation = false, asyncPaidEnabled = true, asyncReplay, asyncError } = {}) {
  const calls = { repository: 0, engine: 0, renderer: 0, async: 0 };
  const audit = [];
  const dependencies = {
...
test("free/light/premium標準modeとdeep明示権利を既存resolverで処理する", async () => {
  ...
  for (const [membership, requested, expected] of cases) {
    ...
    assert.equal(response.statusCode, expected === "free" ? 200 : 202);
    if (expected === "free") assert.equal(calls.engineInput.plan, expected);
    else {
      assert.equal(calls.asyncParams.mode, expected);
...
test("light/deepはHTTP内でengineとrendererを呼ばず202 queuedだけを返す", async () => {
...
test("paid asyncが無効なら同期fallbackせず固定503", async () => {
  const { handler, calls } = setup({ membership: { plan: "light", subscription_status: "active" }, asyncPaidEnabled: false });
  const response = await handler(event());
  assert.equal(response.statusCode, 503);
  assert.equal(body(response).error.code, "READING_ASYNC_PAID_DISABLED");
  assert.deepEqual([calls.engine, calls.renderer, calls.async], [0, 0, 0]);
});

test("completed replayはengine/rendererを呼ばず保存済み結果へ新request_idだけを付ける", async () => {
  const { handler, calls } = setup({ asyncReplay: "completed", membership: { plan: "light", subscription_status: "active" } });
  const response = await handler(event()); const value = body(response);
  assert.equal(response.statusCode, 200); assert.equal(value.status, "completed"); assert.equal(value.request_id, "gateway-request-001");
...
test("deepもasync paid kill switch未設定相当で拒否し旧deep flagへfallbackしない", async () => {
  const { handler, calls } = setup({ deepEnabled: true, asyncPaidEnabled: false, membership: { plan: "premium", subscription_status: "active", deep_enabled: true } });
  const response = await handler(event({ body: JSON.stringify({ name: "架空", birth_date: "1984-12-29", requested_mode: "deep" }) }));
  assert.equal(response.statusCode, 503); assert.equal(body(response).error.code, "READING_ASYNC_PAID_DISABLED"); assert.equal(calls.engine, 0);
});

test("予期しない例外は固定500、request_idはheader/bodyで一致する", async () => {
  const { handler } = setup({ membership: { plan: "light", subscription_status: "active" }, asyncError: new Error("raw provider secret") });
  const response = await handler(event()); const value = body(response);
